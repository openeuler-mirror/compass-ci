...
class TaskProcessor:
    ...
    def bisect_producer(self):
        ...
        test_task = {
            "bad_job_id": 12345, 
            "error_id": "test_errid_123",
            "bisect_status": "wait"
        }
        if self.add_bisect_task(test_task):
            logger.info("✅ 测试任务添加成功")
        else:
            logger.error("❌ 测试任务添加失败")
        ...
        while self.running:
            ...
            try:
                # 数据库连接检查
                logger.debug(f"DEBUG - 数据库连接状态: {self.jobs_db.health_check()}")
                
                # 获取白名单
                errid_white_list = self.get_errid_white_list()
                logger.debug(f"DEBUG - 白名单大小: {len(errid_white_list)}")
                if errid_white_list:
                    logger.debug(f"DEBUG - 白名单样例: {list(errid_white_list)[:3]}")
                ...
                sql_failure = """
                    ...
                    LIMIT 1000
                """
                logger.debug(f"DEBUG - 执行SQL:\n{sql_failure}")
                
                # 执行查询
                result = self.jobs_db.execute_query(sql_failure)
                logger.debug(f"DEBUG - 查询返回 {len(result)} 行结果")
                if result:
                    logger.debug(f"DEBUG - 首行数据: {json.dumps(result[0], indent=2)}")
                ...
                for item in result:
                    # 修复：转换为整数
                    bad_job_id = int(item["id"])
                    errids = item["errid"].split()
                    
                    logger.debug(f"DEBUG - 任务 {bad_job_id} 原始错误IDs: {errids}")
                    
                    # 过滤白名单
                    candidates = set(errids) & errid_white_list if errid_white_list else errids
                    logger.debug(f"DEBUG - 过滤后错误IDs: {candidates}")
                    ...
                    for errid in candidates:
                        ...
                        logger.debug(f"DEBUG - 任务已存在? {bool(existing)}")
                        ...
                        if not existing:
                            ...
                            logger.debug(f"DEBUG - 添加任务: {task_data}")
                            if self.add_bisect_task(task_data):
                                success_count += 1
                
                # 结果日志
                logger.info(f"添加 {success_count}/{len(result)} 新任务")
