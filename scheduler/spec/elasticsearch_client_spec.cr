...
describe  Elasticsearch::Client do
    describe "add job" do
        it "add job without job id success" do
            ...
            respon = es_client.add("/testjobs/job", {"foo" => "bar"}, "1")
        ...
        it "add job with job id success" do
            ...
            raw_es_client = Elasticsearch::API::Client.new( { :host => "localhost", :port => 9200 } )
            raw_es_client.indices.delete({:index => "testjobs"})
            es_client.add("/testjobs/job", {"foo" => "bar", "id" => "3"}, "2")
    ...
    describe "add config" do
        it "add config and get (just as test Elasticsearch)" do
            es_client = Elasticsearch::Client.new("localhost", 9200)
            raw_es_client = Elasticsearch::API::Client.new( { :host => "localhost", :port => 9200 } )
            raw_es_client.indices.delete({:index => "testjobs"})

            data = {:address => "192.168.0.1:1234", :hostname => "1234"}
            test_document = "/testjobs/hostnames"
            es_client.add_config(test_document, data)
            respon = es_client.get_config(test_document, "192.168.0.1:1234")
            respon.should eq("1234")

            raw_es_client.indices.delete({:index => "testjobs"})
        end

        it "add config and get when there has no mac" do
            es_client = Elasticsearch::Client.new("localhost", 9200)
            raw_es_client = Elasticsearch::API::Client.new( { :host => "localhost", :port => 9200 } )
            raw_es_client.indices.delete({:index => "testjobs"})

            data = {:address => "192.168.0.1:1234", :hostname => "1234", :mac => nil}
            test_document = "testjobs/hostnames"
            es_client.add_config(test_document, data)
            respon = es_client.get_config(test_document, "192.168.0.1:1234")
            respon.should eq("1234")
            
            respon = raw_es_client.cat.count({ :index => "testjobs" })
            # 1587092620 03:03:40 0
            # why it is 0?
