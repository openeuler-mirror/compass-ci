...
2. [编写PKGBUILD](https://atomgit.com/openeuler/compass-ci/blob/master/doc/test-guide/write-PKGBUILD.zh.md)（无法通过命令安装的软件如yum install/apt-get add，才需要编写PKGBUILD，可以直接安装的情况下请跳过该步骤）
...
4. 添加待测试仓库 URL 到 upstream仓库
	...
	> **说明：**      
	> [os /os_version /os_arch](https://atomgit.com/openeuler/compass-ci/blob/master/doc/job/fields/os-os_verison-os_arch.md) [os_mount](https://atomgit.com/openeuler/compass-ci/blob/master/doc/job/fields/os_mount.md)
	> [job](https://atomgit.com/openeuler/compass-ci/tree/master/doc/job)
