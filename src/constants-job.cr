...
JOB_STAGE_NAME2ID = {
  ...
  "abort_provider"    => 13,    # auto cancel by provider, due to errors like download/extracting cpio
}
...
JOB_DATA_READINESS_NAME2ID = {
  ...
  "norun"         =>  5,  # canceled, no run, no stats. this state is necessary for wakeup mechanism
...
JOB_HEALTH_NAME2ID = {
  ...
  "abort_provider"                =>  32,   # abort due to error in provider, before qemu/container started
  "abort"                         =>  33,   # pre-condition not met, test script cannot continue
...
MANTI_FULLTEXT_FIELDS = %w[
    ...
    queue all_params_md5 pp_params_md5 tbox_group
...
HOSTS_FULL_TEXT_KEYS = %w[ model_name bios system baseboard cpu memory_info cards network disks ]

# for auto substitution in query input/output
MANTI_JSON_PROPERTIES = MANTI_FULLTEXT_FIELDS + MANTI_FULLTEXT_ARRAY_FIELDS + HOSTS_FULL_TEXT_KEYS
